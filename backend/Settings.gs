...
function generateMonthlyReportPDF(month, year) {
  try {
    ...
    htmlContent += '<h2>Monthly Attendance Report - ' + monthYearLabel + '</h2>';
    
    // Check if there's data
    if (result.reportData.length === 0) {
      htmlContent += '<div class="no-data">No attendance data available for this month.</div>';
    } else {
      // Add employee attendance table
      htmlContent += '<table>';
      htmlContent += '<thead><tr>';
      htmlContent += '<th>Employee ID</th>';
      htmlContent += '<th>Employee Name</th>';
      htmlContent += '<th>Position</th>';
      htmlContent += '<th>On Time</th>';
      htmlContent += '<th>Late</th>';
      htmlContent += '<th>Early Leave</th>';
      htmlContent += '<th>Total Days</th>';
      ...
      htmlContent += '<p><strong>Total On Time:</strong> ' + result.summary.totalOnTime + '</p>';
      htmlContent += '<p><strong>Total Late:</strong> ' + result.summary.totalLate + '</p>';
      htmlContent += '<p><strong>Total Early Leave:</strong> ' + result.summary.totalAbsent + '</p>';
      htmlContent += '<p><strong>Total Employees:</strong> ' + result.reportData.length + '</p>';
      htmlContent += '</div>';
    }
